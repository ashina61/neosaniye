...
const report = focus.map((file) => ({
  file,
  incoming: incoming.get(path.resolve(file)) || [],
  legacyNamed: legacyPattern.test(file),
}));

const removableRoots = report.filter((entry) => entry.legacyNamed && entry.incoming.length === 0).map((entry) => entry.file);
await writeFile('premium-prune-plan.json', JSON.stringify({generatedAt: new Date().toISOString(), report, removableRoots}, null, 2) + '\n');
