 ...
 *
 * The boundary is the END of each clip — the start of the silence that follows
 * it — so the pause belongs to the line coming next, exactly as it does when a
 * one-pass reading is measured. Two paths, one rule.
 *
 ...
 */
export function joinWithGaps(clips, sampleRate, gapSeconds = 0.55) {
  ...
  clips.forEach((clip, i) => {
    ...
    cursor += clip.length;
    if (i < clips.length - 1) {
      boundaries.push(cursor / sampleRate);
      cursor += gap;
    }
  });
