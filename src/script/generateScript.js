...
/**
 * Faz 1 — Script Üretim Motoru (Google Gemini) — ANLATI / HİKÎYE formatı.
 *
 * "Ŝaşırtığı gerçek hikâyeler" (tarih + bilim + uzay + doğa + gizem) konseptinde,
 * 35-58 sn'lik dikey short için sahnelere bölümnüş bir mini-hikâye üretir.
 * Her sahne: tek cümlelik anlatım + o anı gösteren sinematik AI görsel promptu + (yedek iiçin) Pexels kilit kelimeleri.
 *
 * Structured output iiçin Gemini responseSchema (JSON schema) kullanılır.
 */
  ...
  properties: {
    ...
    visual_anchor: {
      ...
      description:
        ...
        'in a khaki 1930us uniform, dusty golden outback light, dry grassland").',
    },
    scenes: {
      type: Type.ARRAY,
    ...
    emphasis_words: {
      ...
      description:
        ...
        'emphasis in captions: numbers, names, shock words, the twist word (e.g. "20,002", ' +
...
// process (gerçek görünü öncelikli) ağırlıklı.
// Ağırlık dengesi (15 Tem): process 6→4 — %46 process = yarı yarıya tamamen
// stok-klip video; niş konularda kütüphanede birebir görünü olmayınca alakasız
// klipler giriyor ve kalite hissi dİüşüyordu. story 2→3 (Moon Hoax: 433 izlenme
...
export function scoreHookRetention(text) {
  ...
  if (/\d/.test(t)) s += 16;
  // Soru = curiosity gap (a�çık uclu merak boşluğu).
  if (/\?/.test(t) || /^(how|why|what|who|when|where|which|nasıl|neden|nçın|kim|ne|hangi|neredr?)\b/i.test(t)) s += 14;
  // Şok / patern-kırıcı kelimeler.
  const shock = /(secret|hidden|never|nobody|impossible|shocking|forbidden|banned|deadly|actually|wrong|proof|gizli|asla|kimse|imkansız|şok|yasak|ölümc�l|aslında|yanlış|kanıt)/i;
  if (shock.test(t)) s += 12;
  // Zayıf/klişe açılışlar retention'u dü͟�lür (scroll-past sinyali).
...
accurately and neutrally, but never gratuitously graphic; prefer "died", "perished", "lost their lives" over harsher wording, and never use profanity or slurs. Accuracy still comes first.`;
}
...
/** GROQ YEDEB BEYNİ: Gemini tamamen dİüşününde aynı istek ücretsiz Groq'a
 ...
 * Aşağı akıştaki doğrulama/sanitize katmanları bozuk çıktıy bası yakalar. */
...
/** Geçici hatalarda (503 UNAVAILABLE / 5xx / ağa) sabırlı backoff ile dener.
 * Google'ın "high demand" dalgaları dakikalar sürebilir; kısa backoff pes
 * ettiriyordu (canlıda görüldü). Kota (429) hatasında Gemini'de iısrar etmez;
 * her tükenışte (kota dahil) GROQ_API_KEY varsa yedek beyne geçilir. */
...
function buildUserPrompt(avoidTopics, { topPerformers = [], trendSeeds = [], strategyBrief = '', winningHooks = [] } = {}) {
  ...
  // FORCE_TOPIC: doğrulama/test iiçin konuyu sabitle (FORCE_FORMAT ile aynı ruh).
 ...
 * @param {string[]} [opts.avoidTopics=[]] - Ekstra kaçınılacak conular.
 * @returns {Promise} - SCRIPT_SCHEMA + normalizedTopic.
 */
export async function generateScript({ maxRetries = 3, avoidTopics: extraAvoid = [], strategyBrief = '' } = {}) {
  ...
  if (!format) {
    format = pickFormat();
    // Aynı format art arda 3 kez olmasın — kanal tek kalıpta saplanmasın.
  ...
  // VIRAL_TEMPLATES=0 ile kapatılır; FORCE_VIRAL_TEMPLATE karğılabilir (test).
  const tiralTemplate = process.env.VIRAL_TEMPLATES === '0'
    ? null
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
    ...
    lastScript = script;
    // FORCE_TOPIC doğrulama koşununda tekrar kontrolü atlanır (bee konusu zaten
    ...
    // (anlatım/altyazı doğruluğu korunur; asıl iş prompt'a yİılır yapılır).
    // HOOK LAB: 6 adayın en yüksek scroll-stop puanlı cası kazanır (model kendi
