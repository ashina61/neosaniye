...
export const TrafficLightShort: React.FC = () => {
  ...
  const boundaryIndex = starts.slice(1, -1).findIndex((cut) => Math.abs(frame - cut) <= 8);
  const transition = boundaryIndex >= 0
    ? {
        kind: beats[boundaryIndex]?.transitionOut ?? 'hard-cut',
        localFrame: frame - starts[boundaryIndex + 1],
      }
    : null;
  ...
  return (
    <AbsoluteFill style={{background: palette.paper, overflow: 'hidden'}}>
      {scene}
      {transition ? <TransitionLayer kind={transition.kind} localFrame={transition.localFrame} /> : null}
